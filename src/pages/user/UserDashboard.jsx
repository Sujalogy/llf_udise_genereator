...
const UserDashboard = () => {
  ...
  const activeData = useMemo(() => {
    ...
    const cacheKey = `${state.selectedState}_${state.selectedDistricts.sort().join('-')}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setTimeout(() => dispatch({ type: ACTIONS.CACHE_DATA, key: cacheKey, data: mockData }), 800);
      }
    };
      ...
      <aside className={`flex-shrink-0 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col transition-all duration-300 ${state.sidebarOpen ? 'w-64' : 'w-0 opacity-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {Object.keys(state.filters).map(st => (
            <button key={st} onClick={() => dispatch({ type: ACTIONS.SELECT_STATE, payload: st })} className={`w-full text-left px-4 py-3 text-sm flex items-center justify-between border-l-4 transition-all ${state.selectedState === st ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 border-blue-600' : 'border-transparent text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
              {st} {state.selectedState === st && <ChevronRight className="w-4 h-4" />}
            </button>
          ))}
        </div>
      </aside>
        ...
        <aside className="w-64 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col animate-in slide-in-from-left-10 duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            ...
            {state.filters[state.selectedState]?.map(dist => {
              ...
              return (
                <div key={dist} onClick={() => dispatch({ type: ACTIONS.TOGGLE_DISTRICT, payload: dist })} className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm cursor-pointer select-none transition-colors ${isSelected ? 'bg-white dark:bg-gray-800 shadow-sm text-blue-600 dark:text-blue-400 ring-1 ring-gray-200 dark:ring-gray-700' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800'}`}>
                  <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-blue-600 border-blue-600' : 'border-gray-400 bg-transparent'}`}>{isSelected && <Check className="w-3 h-3 text-white" />}</div>
                  {dist}
                </div>
              );
            })}
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-50 dark:bg-gray-900/50">
        <header className="h-16 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-800 dark:text-white">School Registry</h1>
              <div className="flex items-center gap-2 text-xs text-gray-500">
                <span>{state.selectedState || "All India"}</span>
                {state.selectedDistricts.length > 0 && <><ChevronRight className="w-3 h-3" /><span>{state.selectedDistricts.length} District(s)</span></>}
              </div>
            </div>
          </div>
